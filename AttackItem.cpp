...
/**
 * Attack Item constructor based on price and name
 * @param price the price of the item
 * @param name the name of the item
 */
AttackItem::AttackItem(int price, string name) {
    set_effect();
    _price = price;
    _name = name;
}
...
/**
 * Attack Item default constructor that sets _effect to 0
 */
 ...
 * @return 'w' to indicate that this is an attack item
 */
char AttackItem::get_type() {
    return 'w';
}
 ...
 */
void AttackItem::observe(WINDOW * game_window, int y_max, int x_max) {
    mvwprintw(game_window, y_max / 2, x_max / 2, "Attack Items Shop is here! You can buy a new Item if you want!");
    mvwprintw(game_window, y_max / 2 + 1, x_max / 2, _name.c_str());

    string temp_str = to_string (get_effect());
    mvwprintw(game_window, y_max / 2 + 2, x_max / 2, "Attack: ");
    ...
    mvwprintw(game_window, y_max / 2 + 4, x_max / 2, "[PRESS B TO BUY OR X TO IGNORE]");
    wrefresh(game_window);
}

/**
 * A method to clear the text of the observed item
 * @param game_window the window to print at
 * @param y_max the maximum height of the window
 * @param x_max the maximum width of the window
 */
void AttackItem::clear_item(WINDOW * game_window, int y_max, int x_max) {
 ...
 * @param x_max the maximum width of the window
 */
void AttackItem::buy(WINDOW * game_window, Party & party, int y_max, int x_max) {
